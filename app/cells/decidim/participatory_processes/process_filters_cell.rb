    ...
    class ProcessFiltersCell < Decidim::ViewModel
      ...
      def current_participatory_processes_path(date_filter, type_filter)
        if Decidim::ParticipatoryProcess.scoped_groups_namespace == ParticipatoryProcessesScoper::ALTERNATIVE_NAMESPACE
          return alternative_filter_link(Decidim::ParticipatoryProcess.scoped_groups_namespace, date_filter, type_filter)
        end

        normal_filter_link(date_filter, type_filter)
      end

      def alternative_filter_link(key, date_filter, type_filter)
        Rails.application.routes.url_helpers.send("#{key}_path", **filter_params(date_filter, type_filter))
      end
